...
enum MicroFeatureGroup: String {
    ...
    case icarus = "Icarus"
    case webOperation = "WebOperation"
    case common = "Common"
...
let SkyNLP: Command = .init(projectName: "SkyNLP", targetType: .framework, templates: [.lenses], libs: ["Foundation", "CoreGraphics", "SkyObject"])

let MysticViewStateBoundedContext: Command = .init(projectName: "MysticViewStateBoundedContext", group: .mystic, targetType: .framework, templates: [.prism], libs: ["ManView", "MysticView", "ZMPhotoView"])

let ManUIModel: Command = .init(projectName: "ManUIModel",
                                group: .applicationServices,
                                targetType: .framework,
                                templates: [.prism, .caseName],
                                libs: ["IcarusViewBoundedContext",
                                       "LabelingAction",
                                       "ManView",
                                       "MysticView",
                                       "MysticViewStateBoundedContext",

                                       "SkyWebViewAction",
                                       "SkyWebViewSideEffectMiddleware",
                                       "WebOperationAction"])
...
let task = Task(
    ...
    // print(SkyNLP.commandStr)
    print(shell(SkyNLP.commandStr.components(separatedBy: " ")))
    print(shell(MysticViewStateBoundedContext.commandStr.components(separatedBy: " ")))
    // print(ManUIModel.commandStr)
    print(shell(ManUIModel.commandStr.components(separatedBy: " ")))
}
